...
public:
  ...
  template <class T, class... Args any_REQUIRES_T(
  ...
  template <class ValueType> any &operator=(ValueType const &value) {
    any(value).swap(*this);
    return *this;
  }
  ...
  template <class ValueType> const ValueType *to_ptr() const {
    return &(static_cast<holder<ValueType> *>(content)->held);
  }

  template <class ValueType> ValueType *to_ptr() {
    return &(static_cast<holder<ValueType> *>(content)->held);
  }
...
private:
  ...
  public:
    ...
    virtual std::type_info const &type() const { return typeid(ValueType); }
...
inline void swap(any &x, any &y) any_noexcept { x.swap(y); }

#if any_CPP11_OR_GREATER

template <class T, class... Args> inline any make_any(Args &&... args) {
